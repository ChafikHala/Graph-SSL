...
#         """Construit l'arbre WL."""
#         print(f"--- Construction WL (Convergence: {force_convergence}) ---")
...
#                 print(f"-> Convergence stable atteinte à l'itération {it-1}.")
#                 break
...
#                 # Arêtes (Non-dirigées pour le BFS)
...
#         """
#         Récupère les noeuds similaires (distance <= delta) SANS doublons et SANS le target.
#         """
...
#         found_graph_nodes = set() # SET pour éviter les doublons [6,6,1,6]
        
#         while queue:
#             curr_tree_node, dist = queue.popleft()
            
#             # 1. Récolter les membres de ce noeud de l'arbre
#             members = self.tree_members.get(curr_tree_node, [])
#             for m in members:
#                 if m != target_node:
#                     found_graph_nodes.add(m)
            
#             # 2. Continuer l'exploration si budget distance > dist
#             if dist < delta:
...
#         """Score basé sur la profondeur du LCA (Lowest Common Ancestor)."""
...
#         return lca_depth / max_depth if max_depth > 0 else 0
...
#             elif n == "root": labels[n] = "ROOT"
#             else: labels[n] = ""
            
#         nx.draw(self._viz_graph, pos, with_labels=True, labels=labels, 
#                 node_color=node_colors, cmap=plt.cm.coolwarm, node_size=600)
#         plt.title("WL Hierarchy Generated")
#         plt.show()
