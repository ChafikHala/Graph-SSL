...
from wl_gcl.src.trainers.train_wl_swav import train_wl_swav
...
from wl_gcl.configs.wl_swav import make_wl_swav_cfg
...
def main() -> None:
    ...
    parser.add_argument(
        "--method",
        choices=["wl", "baseline", "wl_hierarchy", "wl_swav"],
        default="wl",
    ...
    elif args.method == "wl_swav":
        cfg = make_wl_swav_cfg(args.dataset)
        cfg = replace(cfg, model=args.model)
        print(
            f"[RUN] Method=WL-SwAV | "
            f"Dataset={cfg.dataset.upper()} | "
            f"Model={cfg.model.upper()}"
        )
        train_wl_swav(cfg)
